...
// extern iTE_u8 it6664_read_one_block_edid(iTE_u8 port, iTE_u8 block, iTE_u8
// *edid_buffer); extern iTE_u8 ddcwait(iTE_u8 port); extern void
// it6664_var_init(void); extern void it6664_txvar_init(void); extern void
// DefaultEdidSet(void); extern void it6664_EdidMode_Switch(void); extern void
// h2tx_pwron(iTE_u8 port); extern void txrunhdcp2(iTE_u8 port); extern void
// setrx_ksv_list(iTE_u8 devcnt, iTE_u8 depth, iTE_u8 err_devcnt, iTE_u8
// err_depth); extern void SetRxHpd(iTE_u8 sts); extern void hdmi2_work(void);
...
#define mhlrxwr(handle, offset, data)                                          \
  iTE_I2C_WriteByte(handle, RXMHLAddr, offset, data)
#define mhlrxbrd(handle, offset, byteno, rddata)                               \
  iTE_I2C_ReadBurst(handle, RXMHLAddr, offset, byteno, rddata)
#define mhlrxbwr(handle, offset, byteno, rddata)                               \
  iTE_I2C_WriteBurst(handle, RXMHLAddr, offset, byteno, rddata)
#define mhlrxset(handle, offset, mask, data)                                   \
  iTE_I2C_SetByte(handle, RXMHLAddr, offset, mask, data)
// TX
#define h2txrd(handle, port, offset)                                           \
  iTE_I2C_ReadByteP(handle, TXP0Addr, port, offset)
#define h2txwr(handle, port, offset, data)                                     \
  iTE_I2C_WriteByteP(handle, TXP0Addr, port, offset, data)
#define h2txset(handle, port, offset, mask, data)                              \
  iTE_I2C_SetByteP(handle, TXP0Addr, port, offset, mask, data)
#define h2txbrd(handle, port, offset, byteno, rddata)                          \
  iTE_I2C_ReadBurstP(handle, TXP0Addr, port, offset, byteno, rddata)
#define h2txcomwr(handle, offset, data)                                        \
  iTE_I2C_WriteByte(handle, TXComAddr, offset, data)
#define h2txcomrd(handle, offset) iTE_I2C_ReadByte(handle, TXComAddr, offset)
#define h2txcomset(handle, offset, mask, data)                                 \
  iTE_I2C_SetByte(handle, TXComAddr, offset, mask, data)
#define h2txcombwr(handle, offset, byteno, rddata)                             \
  iTE_I2C_WriteBurst(handle, TXComAddr, offset, byteno, rddata)
// SP
#define h2spset(handle, offset, mask, data)                                    \
  iTE_I2C_SetByte(handle, SWAddr, offset, mask, data)
#define h2spwr(handle, offset, data)                                           \
  iTE_I2C_WriteByte(handle, SWAddr, offset, data)
#define h2spbrd(handle, offset, byteno, rddata)                                \
  iTE_I2C_ReadBurst(handle, SWAddr, offset, byteno, rddata)
#define h2sprd(handle, offset) iTE_I2C_ReadByte(handle, SWAddr, offset)
#define h2spbwr(handle, offset, byteno, rddata)                                \
  iTE_I2C_WriteBurst(handle, SWAddr, offset, byteno, rddata)
// RX
#define h2rxwr(handle, offset, data)                                           \
  iTE_I2C_WriteByte(handle, RXP0Addr, offset, data)
#define h2rxrd(handle, offset) iTE_I2C_ReadByte(handle, RXP0Addr, offset)
#define h2rxset(handle, offset, mask, data)                                    \
  iTE_I2C_SetByte(handle, RXP0Addr, offset, mask, data)
#define h2rxbrd(handle, offset, byteno, rddata)                                \
  iTE_I2C_ReadBurst(handle, RXP0Addr, offset, byteno, rddata)
// EDID
#define h2rx_edidwr(handle, offset, data)                                      \
  iTE_I2C_WriteByte(handle, RXEDID_Addr, offset, data)
